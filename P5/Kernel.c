...
-----------------------------  HoareCondition  ---------------------------------
-- Attributions:
-- "Hoare Semantics for Condition Variables" by Harry Porter
-- 
  behavior HoareCondition
    ...
    method Signal( inCount: ptr to int, inSem: ptr to Semaphore, inLock: ptr to Mutex )
      if cnt > 0
        *inCount = *inCount + 1
        sem.Up()
        (*inSem).Down()
    ...
    method Wait( inCount: ptr to int, inSem: ptr to Semaphore, inLock: ptr to Mutex )
      cnt = cnt + 1
      if *inCount > 0
        (*inSem).Up()
      else
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- Initialize array o' Threads, plus that other stuff
        -- Note: You can move these into the header file
        ...
        -- Initialize each in o'Array & set status to UNSUED, or UNUSED if you prefer
        -- KPL is a stupid language. There is no clear way to get i as a string, so you can't name in a loop 
        -- So inelegant... 
        for i = 0 to MAX_NUMBER_OF_PROCESSES-1                
          threadTable [i].Init( "anThread" )
          threadTable [i].status = UNUSED
        endFor        
      
        -- Which means they MUST be on the freeList
        -- Initialize the freeList
        freeList = new List [Thread]
      
        -- Place all threads in threadTable[] on freeList
        j = (threadTable arraySize) - 1
        for i = 0 to j          
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- Remove and return a THREAD from the freeList
        -- if FL is empty, just chill
             
         
        if ( !freeList.IsEmpty() ) -- check FL for emptiness
          thrdPtr = freeList.Remove()
        else -- Hint: It is
          if ( !thrdPtr ) -- HoareSemantics
            aThreadBecameFree.Wait( &nextCount, &nextSem, &threadManagerLock ) -- Just chill
            thrdPtr = freeList.Remove()
          endIf
          /*
          while ( !thrdPtr ) -- Check to see if we have a pointer yet            
            if ( threadManagerLock.IsHeldByCurrentThread() ) -- Apparently this is a thing. Thanks Blitz.
              thrdPtr = freeList.Remove()
            endIf
            if ( !thrdPtr )
              aThreadBecameFree.Wait( &nextCount, &nextSem, &threadManagerLock ) -- Just chill
        ...
        -- Also, change the status to JUST_CREATED
        thrdPtr.status = JUST_CREATED
        
        -- Something about the last hippie out turning on the lights again        
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- Add a THREAD back to the FL and signal
        freeList.AddToEnd(th)
        aThreadBecameFree.Signal( &nextCount, &nextSem, &threadManagerLock )  
        
        -- Change status back to UNSUED, or whatever. I should fix that.
        th.status = UNUSED
        
        -- Repeat bohmenian joke from earlier
        -- Relevant: https://www.youtube.com/watch?v=dQw4w9WgXcQ
        -- Huh, well it says it works, so I guess it works.
        --threadManagerLock.Unlock()
        if nextCount > 0
          nextSem.Up()
        else
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        -- This is where I complain again about KPL
        for i = 0 to MAX_NUMBER_OF_PROCESSES-1                
          processTable [i].Init()
          
          -- Initialize all the PCBs in that array
          processTable [i].status = FREE
        endFor        
        
        -- and place them on the freeList
        freeList = new List [ProcessControlBlock]
        j = (processTable arraySize) - 1
        for i = 0 to j          
          freeList.AddToEnd( &processTable[i] )
        endFor
        
        -- Initialize the processManagerLock
        processManagerLock = new Mutex
        processManagerLock.Init()        
        
        -- Initialize the aProcessBecameFree and aProcessDied condition variables
        ...
        -- Don't forget your locks on GetANewProcess and FreeProcess
        -- Okay, Mom.
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        -- 
        -- This method returns a new PCB; it will wait
        -- until one is available.
        -- 
        ...
        if ( !freeList.IsEmpty() ) -- check FL for emptiness
          pcbPtr = freeList.Remove()
        else -- Hint: It is
          while ( !pcbPtr ) -- Check to see if we have a pointer yet 
            if ( processManagerLock.IsHeldByCurrentThread() ) -- Apparently this is a thing. Thanks Blitz.
              pcbPtr = freeList.Remove()
            endIf
            if ( !pcbPtr )
              aProcessBecameFree.Wait( &processManagerLock ) -- Just chill
            endIf
          endWhile
        endIf
        
        -- Also, change the status to ACTIVE
        pcbPtr.status = ACTIVE
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- Change status to FREE and add it to the FL
        -- Pretty much duplicates the thread version
        processManagerLock.Lock()
        p.status = FREE
        freeList.AddToEnd(p)
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
        var 
          ...
          -- Acquire frameManagerLock
          frameManagerLock.Lock()
          aPageTable.SetToThisPageTable()
          
          -- Wait on newFramesAvailable
          ...
          for i = 0 to numFramesNeeded - 1
            fzs = framesInUse.FindZeroAndSet()
            if (fzs != -1)
              frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (fzs * PAGE_SIZE)
              aPageTable.SetFrameAddr (i, frameAddr)
            else
              print("Not enough frames or something")
            endIf
          endFor              
          
          -- Adjust the number of numberFreeFrames
          numberFreeFrames = numberFreeFrames - numFramesNeeded
          
          -- Set aPageTable.numberOfPages = frames allocated
      ...
      var
        i, frameAddr, rtn, fnum: int
        
        -- Acquire Lock
        frameManagerLock.Lock()
        aPageTable.SetToThisPageTable()
        
        -- Count Frames to return
        rtn = aPageTable.numberOfPages
        
        -- Go through the page table to see which frames it possessed
        for i = 0 to rtn - 1
          frameAddr = aPageTable.ExtractFrameAddr(i)
          fnum = (frameAddr - PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME) / PAGE_SIZE
          -- Clear 'the bit'
          framesInUse.ClearBit(fnum)
          numberFreeFrames = numberFreeFrames + 1
        endFor        
        
        -- Adjust number of free frames and signal        
        -- Do a Broadcast yada yada something from Parlor
        newFramesAvailable.Broadcast( &frameManagerLock )
        aPageTable.numberOfPages = aPageTable.numberOfPages - rtn        
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int      
      ...
      newAddrSpace.Init ()
      n = currentThread.myProcess.addrSpace.GetStringFromVirtual (&strBuffer, filename asInteger, MAX_STRING_SIZE)
...
-----------------------------  OpenFile  ---------------------------------
  ...
  function InitFirstProcess ()
    var fpth: ptr to Thread
      fpth = threadManager.GetANewThread ()
      fpth.Init ("UserProgram")
      fpth.Fork (StartUserProcess, 0)
  
  endFunction
  
    -----------------------------  StartUserProcess  ---------------------------------
  
  function StartUserProcess (invar:int)
